...
{{
    ...
    {{
        ...
        foreach (var (name, _) in info.EnumMembers)
        {
            ...
            _ = sb.AppendLine($@"            ""{nameCased}"" => {info.EnumName}.{name},");
        }

        if (info.UseDefaultValue && info.DefaultValue != null)
        {
            _ = sb.AppendLine($@"            _ => ({info.EnumName}){info.DefaultValue}");
        }
        else
        {
            _ = sb.AppendLine($@"            _ => throw new ArgumentOutOfRangeException(nameof(value), value, ""Value is not a valid {info.EnumName} name."")");
        }
    ...
    {{
        ...
        {
            string nameCased = ApplyCaseConversion(name, info.StringCaseConversion);
            _ = sb.AppendLine($@"            {info.EnumName}.{name} => ""{nameCased}"",");
        }
        ...
        if (info.UseDefaultValue && info.DefaultValue != null)
        {
            ...
            {
                string nameCased = ApplyCaseConversion(Name, info.StringCaseConversion);
                _ = sb.AppendLine($@"            _ => ""{nameCased}""");
            }
            else
            {
                _ = sb.AppendLine($@"            _ => throw new ArgumentOutOfRangeException(nameof(value), value, ""Value is not a valid {info.EnumName}."")");
            }
        }
        else
        {
